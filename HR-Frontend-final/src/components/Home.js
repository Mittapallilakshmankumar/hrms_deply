...
useEffect(() => {
  ...
  fetch(`http://127.0.0.1:8000/api/leave/list/?user_id=${userId}`)
    .then(res => res.json())
    .then(data => {
...
function TrackerPage() {
  ...
  const getHours = (inTime, outTime, summary) => {
    ...
    // ✅ Detect auto logout
    if (summary && summary.toLowerCase().includes("auto")) {
      result += " (Auto Logout)";
    }
            ...
            {data.map((item, i) => {
              ...
              return (
                <tr key={i} className="text-center border-t">
                  ...
                  <td className="p-2">
                    {getHours(checkIn, checkOut, item.summary)}
                  </td>
