...
export default function PortalPage() {
  ...
  const handleRoleSelect = (selectedRole) => {
    navigate("/login", { state: { portalRole: selectedRole } });
  };

  const goBackToHRPortal = () => {
    clearAuthTokens();
      ...
      <header className="sticky top-4 z-20 mx-auto mb-6 max-w-6xl rounded-3xl border border-white/70 bg-white/80 shadow-sm backdrop-blur">
        <div className="flex w-full items-center justify-between px-6 py-4">
          <div>
            <button
              type="button"
              onClick={goBackToHRPortal}
              className="mb-3 inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-blue-700 shadow-sm transition hover:bg-blue-50 hover:text-blue-800"
            >
